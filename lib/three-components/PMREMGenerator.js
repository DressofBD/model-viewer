...
const setup = (cubeTarget) => {
    ...
    const cubeUVRenderTarget = new WebGLRenderTarget(3 * (Math.pow(2, maxLods) + 2), 4 * maxLods + 2 * (Math.pow(2, maxLods + 1) - 1), params);
    cubeUVRenderTarget.texture.name = 'PMREMCubeUVPacker.cubeUv';
    cubeUVRenderTarget.texture.mapping = CubeUVReflectionMapping;
    return { cubeUVRenderTarget, cubeLods, meshes };
};
const appendLodMeshes = (meshes, target, sizeLod, offset) => {
